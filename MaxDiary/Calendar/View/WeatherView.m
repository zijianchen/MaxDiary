...
// 在使用自定义View的时候，用initWithFrame方法用来初始化并返回一个新的视图对象
...
- (void)creatUI {
    ...
    _cityBtn = [UIButton buttonWithType:UIButtonTypeCustom]; // 城市名称
    _cityBtn.frame = CGRectMake(80, 20, kScreenWidth-160, 44);
    _cityBtn.tag = 10;
    _cityBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
    _cityBtn.titleLabel.font = [UIFont boldSystemFontOfSize:30];
    [self addSubview:_cityBtn];
    
    UIImageView *weatherImg = [[UIImageView alloc]initWithFrame:CGRectMake(kScreenWidth/2-50*widthPix, CGRectGetMaxY(_cityBtn.frame)+30, 100*widthPix, 100*heightPix)]; // 当前天气图标
    weatherImg.tag = 11;
